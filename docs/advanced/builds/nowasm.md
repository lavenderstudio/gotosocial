...
On [supported platforms](../../getting_started/releases.md#supported-platforms), GoToSocial uses the WebAssembly runtime [Wazero](https://wazero.io/) to sandbox the `ffmpeg`, `ffprobe`, and SQlite driver WebAssembly binaries, allowing these applications to be packaged and run inside the GoToSocial binary, without requiring admins to install + manage any external dependencies.
...
However! To enable folks to run **experimental, unsupported deployments of GoToSocial**, we expose the `nowasm` build tag, which can be used to compile a build of GoToSocial that does not use Wazero or WASM at all. A GoToSocial binary built with `nowasm` will use system `ffmpeg` and `ffprobe` binaries for media processing. Additionally, it will use a transpiled C-to-Go `moderncsqlite3` driver rather than the WASM-based driver.

The `nowasm` tag has the same effect as building with both the `moderncsqlite3` and `nowasmffmpeg` tags, which eliminate the WASM-based SQLite driver and media processing, respectively.
...
In order to run a version of GoToSocial built in this way, you must ensure that `ffmpeg` and `ffprobe` are installed on the host. This is usually as simple as running a command like `doas pkg_add ffmpeg` (OpenBSD), or `sudo apt install ffmpeg` (Debian etc.). 
