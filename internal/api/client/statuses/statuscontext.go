...
func (m *Module) StatusContextGETHandler(c *httputil.Context) {
	...
	threadContext, errWithCode := m.processor.Status().ContextGet(c, authed.Account, targetStatusID)
	if errWithCode != nil {
		apiutil.ErrorHandler(c, m.templates, errWithCode)
		return
	}

	httputil.JSON(c, http.StatusOK, threadContext)
}
