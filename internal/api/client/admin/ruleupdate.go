...
func (m *Module) RulePATCHHandler(c *httputil.Context) {
	...
	ruleID, errWithCode := apiutil.ParseID(c.PathValue(apiutil.IDKey))
	if errWithCode != nil {
		apiutil.ErrorHandler(c, m.templates, errWithCode)
		return
	}
	...
	apiRule, errWithCode := m.processor.Admin().RuleUpdate(c, ruleID, form)
	if errWithCode != nil {
		apiutil.ErrorHandler(c, m.templates, errWithCode)
		return
	}

	httputil.JSON(c, http.StatusOK, apiRule)
}
