...
func (m *Module) EmojiCreatePOSTHandler(c *httputil.Context) {
	authed, errWithCode := apiutil.TokenAuth(c, apiutil.AuthRequirements{
		Token:   true,
		App:     true,
		User:    true,
		Account: true,
		Scope:   []apiutil.Scope{apiutil.ScopeAdminWriteCustomEmojis},
	})
	if errWithCode != nil {
	...
	apiEmoji, errWithCode := m.processor.Admin().EmojiCreate(c, authed.Account, form)
	if errWithCode != nil {
		apiutil.ErrorHandler(c, m.templates, errWithCode)
		return
	}

	httputil.JSON(c, http.StatusOK, apiEmoji)
}
