...
func (m *Module) ExportBlocksGETHandler(c *httputil.Context) {
	authed, errWithCode := apiutil.TokenAuth(c, apiutil.AuthRequirements{
		Token:   true,
		App:     true,
		User:    true,
		Account: true,
		Scope:   []apiutil.Scope{apiutil.ScopeReadBlocks},
	})
	if errWithCode != nil {
	...
	if _, errWithCode := apiutil.NegotiateAccept(c, apiutil.CSVHeaders...); errWithCode != nil {
		apiutil.ErrorHandler(c, m.templates, errWithCode)
		return
	}
	...
	httputil.EncodeCSVResponse(c, http.StatusOK, records)
}
