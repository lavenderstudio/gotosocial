...
func (m *Module) AccountBlockPOSTHandler(c *httputil.Context) {
	authed, errWithCode := apiutil.TokenAuth(c, apiutil.AuthRequirements{
		Token:   true,
		App:     true,
		User:    true,
		Account: true,
		Scope:   []apiutil.Scope{apiutil.ScopeWriteBlocks},
	})
	if errWithCode != nil {
	...
	relationship, errWithCode := m.processor.Account().BlockCreate(c, authed.Account, targetAcctID)
	if errWithCode != nil {
		apiutil.ErrorHandler(c, m.templates, errWithCode)
		return
	}
