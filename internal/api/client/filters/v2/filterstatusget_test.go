...
func (suite *FiltersTestSuite) getFilterStatus(
	...
	// create the request
	req := httptest.NewRequest(http.MethodGet, config.GetProtocol()+"://"+config.GetHost()+"/api/"+filtersV2.StatusPath+"/"+filterStatusID, nil)
	...
	// trigger the handler
	suite.filtersModule.FilterStatusGETHandler(c)
