...
func (suite *FiltersTestSuite) deleteFilterStatus(
	...
	// create the request
	req := httptest.NewRequest(http.MethodDelete, config.GetProtocol()+"://"+config.GetHost()+"/api/"+filtersV2.StatusPath+"/"+filterStatusID, nil)
	...
	c.SetPathValue("id", filterStatusID)

	// trigger the handler
	suite.filtersModule.FilterStatusDELETEHandler(c)
