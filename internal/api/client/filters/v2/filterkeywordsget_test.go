...
func (suite *FiltersTestSuite) getFilterKeywords(
	...
	// create the request
	req := httptest.NewRequest(http.MethodGet, config.GetProtocol()+"://"+config.GetHost()+"/api/"+filtersV2.BasePath+"/"+filterID+"/keywords", nil)
	...
	// trigger the handler
	suite.filtersModule.FilterKeywordsGETHandler(c)
