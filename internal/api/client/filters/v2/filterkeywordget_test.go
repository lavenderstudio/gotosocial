...
func (suite *FiltersTestSuite) getFilterKeyword(
	...
	// create the request
	req := httptest.NewRequest(http.MethodGet, config.GetProtocol()+"://"+config.GetHost()+"/api/"+filtersV2.KeywordPath+"/"+filterKeywordID, nil)
	...
	// trigger the handler
	suite.filtersModule.FilterKeywordGETHandler(c)
