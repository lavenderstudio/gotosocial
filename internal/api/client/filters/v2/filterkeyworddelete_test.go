...
func (suite *FiltersTestSuite) deleteFilterKeyword(
	...
	// create the request
	req := httptest.NewRequest(http.MethodDelete, config.GetProtocol()+"://"+config.GetHost()+"/api/"+filtersV2.KeywordPath+"/"+filterKeywordID, nil)
	...
	// trigger the handler
	suite.filtersModule.FilterKeywordDELETEHandler(c)
