...
func (suite *FiltersTestSuite) getFilter(
	...
	// create the request
	req := httptest.NewRequest(http.MethodGet, config.GetProtocol()+"://"+config.GetHost()+"/api/"+filtersV1.BasePath+"/"+filterKeywordID, nil)
	...
	// trigger the handler
	suite.filtersModule.FilterGETHandler(c)
