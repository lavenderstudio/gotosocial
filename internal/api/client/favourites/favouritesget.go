...
func (m *Module) FavouritesGETHandler(c *httputil.Context) {
	authed, errWithCode := apiutil.TokenAuth(c, apiutil.AuthRequirements{
		Token:   true,
		App:     true,
		User:    true,
		Account: true,
		Scope:   []apiutil.Scope{apiutil.ScopeReadFavourites},
	})
	if errWithCode != nil {
	...
	resp, errWithCode := m.processor.Timeline().FavedTimelineGet(c, authed, maxID, minID, limit)
	if errWithCode != nil {
		apiutil.ErrorHandler(c, m.templates, errWithCode)
		return
	}
