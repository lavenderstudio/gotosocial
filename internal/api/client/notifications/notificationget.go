...
func (m *Module) NotificationGETHandler(c *httputil.Context) {
	...
	targetNotifID, errWithCode := apiutil.ParseID(c.PathValue(apiutil.IDKey))
	if errWithCode != nil {
		apiutil.ErrorHandler(c, m.templates, errWithCode)
		return
	}

	resp, errWithCode := m.processor.Timeline().NotificationGet(c, authed.Account, targetNotifID)
	if errWithCode != nil {
		apiutil.ErrorHandler(c, m.templates, errWithCode)
		return
	}
