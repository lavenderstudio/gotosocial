...
func (m *Module) NotificationsClearPOSTHandler(c *httputil.Context) {
	authed, errWithCode := apiutil.TokenAuth(c, apiutil.AuthRequirements{
		Token:   true,
		App:     true,
		User:    true,
		Account: true,
		Scope:   []apiutil.Scope{apiutil.ScopeWriteNotifications},
	})
	if errWithCode != nil {
	...
	errWithCode = m.processor.Timeline().NotificationsClear(c, authed)
	if errWithCode != nil {
		apiutil.ErrorHandler(c, m.templates, errWithCode)
		return
	}
