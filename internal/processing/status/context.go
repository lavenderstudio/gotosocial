...
func (p *Processor) contextGet(
	...
	*internalThreadContext,
	...
	// Specifically look for visible target status in thread, which permits
	...
	// Don't thread for boosts/reblogs.
	if targetStatus.BoostOfID != "" {
		err := gtserror.New("target status is a boost wrapper / reblog")
		return nil, gtserror.NewErrorNotFound(err)
	}

	// Fetch up to the top of the thread (status ascendants).
	...
	// Fetch from down to the bottom of the thread (status descendants).
